...
For a matrix \( z \) with rows representing sets of scores, the softmax function is computed as:
\[
\text{softmax}(z_{i,j}) = \frac{e^{z_{i,j} - \max(z_{:,j})}}{\sum_{k} e^{z_{k,j} - \max(z_{:,j})}}
\]
